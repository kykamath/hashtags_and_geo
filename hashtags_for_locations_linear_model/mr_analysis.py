...
UTM_OBJECT_WITH_MIN_COMMON_HASHTAGS= 100
...
class TweetStats(ModifiedMRJob):
    ...
    def map_final_tweet_to_tweet_stats(self): yield 'total_tweets', self.num_of_tweets
...
class HashtagsExtractor(ModifiedMRJob):
    ...
    def jobs_to_extract_hashtags_at_varying_accuracies(self):
        return [self.mr(
            ...
            mapper_final = self.map_final_tweet_to_hashtag_object,
            reducer = self.red_tuo_hashtag_and_hashtag_objects_to_combined_hashtag_object_at_varying_accuracies
        )]
...
class HashtagsDistributionInUTM(ModifiedMRJob):
    ...
    def map_hashtag_object_to_dist_in_utm(self, hashtag, hashtag_object):
        if False: yield # I'm a generator!
        for occurrence_time, utm_id in hashtag_object['ltuo_occ_time_and_occ_utm_id']:
    ...
    def red_tuo_accuracy_and_hashtag_dists_to_accuracy_and_dist(self, accuracy, hashtag_counts):
        yield accuracy, {'accuracy': accuracy, 'hashtag_distribution': list(hashtag_counts)}
    def jobs_to_get_utm_accuracy_distribution(self):
        return self.hashtags_extractor.jobs_to_extract_hashtags_at_varying_accuracies() +\
                [self.mr(
                        mapper = self.map_hashtag_object_to_dist_in_utm,
                        mapper_final = self.map_final_hashtag_object_to_dist_in_utm,
                        reducer = self.red_tuo_utm_id_and_hashtag_counts_to_accuracy_and_hashtag_dist
                    )
                ] +\
                [self.mr(
    ...
    def steps(self): return self.jobs_to_get_utm_accuracy_distribution()
...
class HashtagsByUTMId(ModifiedMRJob):
    '''
        utm_object = {'utm_id': utm_id
                      ...
                      'mf_nei_utm_id_to_common_h_count': mf_nei_utm_id_to_common_h_count
                    }
    '''
    DEFAULT_INPUT_PROTOCOL='raw_value'
    def __init__(self, *args, **kwargs):
    ...
    def map_hashtag_object_to_utm_object(self, hashtag, hashtag_object):
        if False: yield # I'm a generator!
        so_utm_ids = set()
        for occ_time, occ_utm_id in hashtag_object['ltuo_occ_time_and_occ_utm_id']:
            ...
            for utm_id in so_utm_ids:
                for nei_utm_id in so_utm_ids:
                    if utm_id!=nei_utm_id:
                        if nei_utm_id not in self.mf_utm_id_to_mf_nei_utm_id_and_common_h_count[utm_id]:
                            self.mf_utm_id_to_mf_nei_utm_id_and_common_h_count[utm_id][nei_utm_id]=0.0
                        self.mf_utm_id_to_mf_nei_utm_id_and_common_h_count[utm_id][nei_utm_id]+=1.0
    def map_final_hashtag_object_to_utm_object(self):
        for utm_id, mf_hashtag_to_count in self.mf_utm_id_to_mf_hashtag_to_count.iteritems():
            utm_object = {
                           'mf_hashtag_to_count': mf_hashtag_to_count,
                           'total_hashtag_count': self.mf_utm_id_to_total_hashtag_count[utm_id]
                        }
            if UTM_OBJECT_WITH_NEIGHBOR_INFO:
    ...
    def _get_valid_combined_utm_object(self, utm_id, utm_objects):
        combined_utm_object = {'utm_id': utm_id,
                               ...
                               'mf_nei_utm_id_to_common_h_count' : defaultdict(float)
                               }
        for utm_object in utm_objects:
            ...
            if utm_object['total_hashtag_count']:
                combined_utm_object['total_hashtag_count']+=utm_object['total_hashtag_count']
            if UTM_OBJECT_WITH_NEIGHBOR_INFO:
                for nei_utm_id, common_h_count in utm_object['mf_nei_utm_id_to_common_h_count'].iteritems():
                    combined_utm_object['mf_nei_utm_id_to_common_h_count'][nei_utm_id]+=common_h_count
        if UTM_OBJECT_WITH_NEIGHBOR_INFO:
            for nei_utm_id in combined_utm_object['mf_nei_utm_id_to_common_h_count'].keys()[:]:
                if combined_utm_object['mf_nei_utm_id_to_common_h_count'][nei_utm_id] < \
                                                                                    UTM_OBJECT_WITH_MIN_COMMON_HASHTAGS:
        ...
        if combined_utm_object['total_hashtag_count'] >= MIN_HASHTAG_OCCURRENCES_PER_UTM_ID:
            return combined_utm_object
    def red_tuo_utm_id_and_utm_objects_to_combined_utm_object(self, utm_id, utm_objects):
        combined_utm_object = self._get_valid_combined_utm_object(utm_id, utm_objects)
    ...
    def jobs_to_get_utm_object(self):
        return self.hashtags_extractor.jobs_to_extract_hashtags() +\
                [self.mr(
                    ...
                    reducer = self.red_tuo_utm_id_and_utm_objects_to_combined_utm_object
                )]
    def steps(self): return self.jobs_to_get_utm_object()
...
class HastagsWithUTMIdObject(ModifiedMRJob):
    ...
    def map_utm_object_to_hashtag_with_utm_object(self,
        ...
        for hashtag, count in utm_object['mf_hashtag_to_count'].iteritems():
            if utm_id not in self.mf_hashtag_to_mf_utm_id_to_hashtag_occurrences[hashtag]:
                self.mf_hashtag_to_mf_utm_id_to_hashtag_occurrences[hashtag][utm_id] = 0.0
            self.mf_hashtag_to_mf_utm_id_to_hashtag_occurrences[hashtag][utm_id]+=count
    def map_final_utm_object_to_hashtag_with_utm_object(self):
        for hashtag, mf_utm_id_to_hashtag_occurrences in\
            ...
            hashtag_with_utm_object = {
                                       'hashtag' : hashtag,
                                       'mf_utm_id_to_hashtag_occurrences': mf_utm_id_to_hashtag_occurrences
                                    }
    ...
    def red_tuo_h_and_hashtag_with_utm_object_to_h_and_com_h_with_utm_object(
        ...
        for hashtag_with_utm_object in hashtag_with_utm_objects:
            for utm_id, hashtag_occurrences in hashtag_with_utm_object['mf_utm_id_to_hashtag_occurrences'].iteritems():
    ...
    def jobs_to_get_hashtags_with_utm_id_object(self):
        return self.hashtags_by_utm_id.jobs_to_get_utm_object() + \
                [self.mr(
                    ...
                    mapper_final = self.map_final_utm_object_to_hashtag_with_utm_object,
                    reducer = self.red_tuo_h_and_hashtag_with_utm_object_to_h_and_com_h_with_utm_object
                )]
    def steps(self):

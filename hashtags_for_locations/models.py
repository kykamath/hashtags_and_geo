    ...
    def follow_the_leader(map_from_model_to_cumulative_losses, **conf):
        if not map_from_model_to_cumulative_losses: return random.sample(conf['modelsInOrder'], 1)[0]
            ...
            for model_id in reversed(conf['modelsInOrder']): 
                if model_id in map_from_model_to_cumulative_losses: 
                    tuple_of_model_id_and_cumulative_loss = min([tuple_of_model_id_and_cumulative_loss, (model_id, map_from_model_to_cumulative_losses[model_id])], key=itemgetter(1))
...
#        map(lambda modelId: GeneralMethods.runCommand('rm -rf %s'%self.getModelFile(modelId)), self.predictionModels)
        ...
        while currentTime<end_time:
            ...
            for i in range(self.historyTimeInterval.seconds/TIME_UNIT_IN_SECONDS):
                historicalTimeUnit = currentTime-i*timeUnitDelta
                if historicalTimeUnit not in historicalTimeUnitsMap: 
                    historicalTimeUnitsMap[historicalTimeUnit]=Propagations(
                                                                            historicalTimeUnit,
                                                                            self.historyTimeInterval
                                                                            )
                historicalTimeUnitsMap[historicalTimeUnit].update(currentOccurrences)
            for i in range(self.predictionTimeInterval.seconds/TIME_UNIT_IN_SECONDS):
                predictionTimeUnit = currentTime-i*timeUnitDelta
                if predictionTimeUnit not in predictionTimeUnitsMap:
                    predictionTimeUnitsMap[predictionTimeUnit]=Propagations(
                                                                            predictionTimeUnit,
...
##            for model_id in self.predictionModels:
##                self.conf['noOfTargetHashtags'] = no_of_hashtags
##                print '* rm -rf %s'%self.getModelFile(model_id)
##                GeneralMethods.runCommand('rm -rf %s'%self.getModelFile(model_id))
#
##        map(lambda modelId: GeneralMethods.runCommand('rm -rf %s'%self.getModelFile(modelId)), self.predictionModels)
##        hard_end_time = self.conf.get('hard_end_time', None)
...
##                                print location, best_model_id, model_id_selected_by_learning_model, metric_score, map_from_location_to_learned_metric_score[location]
                            for model_id, metric_score in tuo_model_id_and_metric_score: 
                                LEARNING_MODEL_METHODS[learning_model_id][LearningWithExpertAdviceModels.MODEL_SCORING_FUNCTION](
                                                                                                                                     model_selection_histories[learning_model_id], 
                                                                                                                                     location, model_id, metric_id, 
                                                                                                                                     Experiments._get_metric_loss_score(metric_id, metric_score)
                                                                                                                                 )
                        iterationData = {'conf': self._getSerializableConf(), 
                                         'tu': GeneralMethods.getEpochFromDateTimeObject(time_unit_when_models_pick_hashtags), 
                                         'modelId': learning_model_id, 'metricId': metric_id, 
                                         'scoresPerLattice': mf_location_to_learned_metric_score}
                        iteration_weights = {'conf': self._getSerializableConf(), 
                                             'tu': GeneralMethods.getEpochFromDateTimeObject(time_unit_when_models_pick_hashtags), 
                                             'modelId': learning_model_id, 'metricId': metric_id, 
                                             'location_weights': dict([(location, model_selection_history[metric_id]) 
                                                                       for location, model_selection_history in 
                                                                        model_selection_histories[learning_model_id].map_from_location_to_model_selection_history.iteritems()
                                                                        ])
                                             }
                        FileIO.writeToFileAsJson(iterationData, self.getModelFile(learning_model_id))
                        FileIO.writeToFileAsJson(iteration_weights, self.getModelWeightsFile(learning_model_id))
...
#        historicalTimeIntervals = map(lambda i: i*TIME_UNIT_IN_SECONDS, range(1,13))
        ...
        for historical_time_interval in historicalTimeIntervals:
            conf = dict(historyTimeInterval = timedelta(seconds=historical_time_interval), predictionTimeInterval = timedelta(seconds=1*TIME_UNIT_IN_SECONDS), noOfTargetHashtags=2)
            experiments = Experiments(startTime, endTime, outputFolder, predictionModels, evaluationMetrics, **conf)
            iteration_results = experiments.loadExperimentsData()
            metric_values_for_model = defaultdict(dict)
            for _, data_for_models in iteration_results.iteritems():
...
#    Experiments.generateDataToDeterminePerformanceWithExpertAdvice(predictionModels, evaluationMetrics, startTime, endTime, outputFolder)
    Experiments.generateDataForLinearRegression(predictionModels, evaluationMetrics, startTime, endTime, outputFolder)

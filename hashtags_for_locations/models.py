...
#        map(lambda modelId: GeneralMethods.runCommand('rm -rf %s'%self.getModelFile(modelId)), self.predictionModels)
    ...
    def generateDataForVaryingNumberOfHastags():
        ...
        conf = dict(historyTimeInterval = timedelta(seconds=1*TIME_UNIT_IN_SECONDS), predictionTimeInterval = timedelta(seconds=5*TIME_UNIT_IN_SECONDS), noOfHashtagsList=noOfHashtagsList)

...
def parseData(line):
    ...
    if len(data)==7: return {'id':int(data[1]), 'u': int(data[0]), 'l': [float(data[2]), float(data[3])], 't': datetime.strptime(data[4], '%Y-%m-%d %H:%M:%S'), 'x': data[5], 'lid': data[6]}
...
#    for i, data in enumerate(FileIO.iterateLinesFromFile('../data/checkin_data.txt')):
        print i
        FileIO.writeToFileAsJson(data, checkinsJSONFile)
